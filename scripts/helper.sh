...
function cdt-directory-prompt() {
  if [[ -z $CDT_DIR_PROMPT ]]; then
    ...
    while true; do
      if [[ $NONINTERACTIVE != true ]]; then
        printf "Is EOSIO.CDT installed in the default location? $HOME/eosio/eosio.cdt (y/n)" && read -p " " PROCEED
      fi
      echo ""
      case $PROCEED in
        "" )
  ...
  export CDT_INSTALL_DIR="${CDT_DIR_PROMPT:-$HOME/eosio/eosio.cdt}"
}
